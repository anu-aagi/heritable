...
#' @export
H2.asreml <- function(model, term_genotype = NULL, method = c("cullis", "oakey")) {
  method <- match.arg(method)
  # if model has not converged, warn
  if(model$converge) cli::cli_warn("The input model has not converged")
  H2 <- switch(method,
               cullis = H2_cullis.asreml(model, term_genotype),
               H2.default())
...
#' @export
H2_cullis.asreml <- function(model, term_genotype = NULL) {
  vc_g <- asreml::summary.asreml(model)$varcomp[term_genotype, 'component']
  # need to supress output from below
  vdBLUP_mat <- asreml::predict.asreml(model,
                                       classify = term_genotype,
                                       only = term_genotype,
                                       sed = TRUE)$sed^2
