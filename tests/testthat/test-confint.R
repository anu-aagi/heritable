...
test_that("Confint works", {
  ...
  # Compare results
  plot_df <- data.frame(
    lme4_resample = attr(ci_lme4, "boot_mod")$t |> drop(),
  ...
  expect_true(
    ks.test(plot_df$lme4_fix, plot_df$asreml_fix)$p.value > 0.05 &
      ks.test(plot_df$lme4_resample, plot_df$asreml_resample)$p.value > 0.05
  )
