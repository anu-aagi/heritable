test_that("Refactored asreml works", {
  ...
  expect_all_true(
    truth[["marginal"]] ==
      H2(model, "gen", methods, options = list(check = FALSE))
  )

  expect_all_true(
    truth[["partial"]] ==
      H2(model, "gen", methods, marginal = FALSE, options = list(check = FALSE))
  )
  ...
  expect_all_true(
    abs(
      H2(model, "gen", methods, options = list(check = FALSE)) -
        H2(model_lme4, "gen", methods, options = list(check = FALSE))
    ) <
      1e-5
  )
  ...
  expect_all_true(
    abs(
      H2(
        ...
        "gen",
        methods,
        stratification = data.frame(loc = "L1"),
        options = list(check = FALSE)
      ) -
        H2(
          model_lme4,
