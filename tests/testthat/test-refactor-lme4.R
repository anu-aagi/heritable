test_that("Refactored lme4 works", {
  ...
  truth <- readRDS(test_path("fixtures/refactor_lme4_result.rds"))
  methods <- c("Cullis", "Oakey", "Delta", "Standard")

  # Test model 1, with unstructured environmental variance.
  model1 <- lme4::lmer(
    y ~ rep + (0 + loc | gen) + (1 | gen),
    data = lettuce_phenotypes
  )

  expect_all_true(
    truth[["m1_marginal"]] ==
      H2(model1, "gen", methods, options = list(check = FALSE))
  )
  ...
  # Test model 2, with simple environmental variance.
  model2 <- lme4::lmer(y ~ rep + (1 | gen * loc), data = lettuce_phenotypes)

  expect_all_true(
    truth[["m2_marginal"]] ==
      H2(model2, "gen", methods, options = list(check = FALSE))
  )
  ...
  expect_equal(
    max(
      lme4::getME(model1, "Z")[rownames(stra), target_trms] -
        build_new_Z(
          model1,
          "gen",
          new_data = data.frame("loc" = "L1", "rep" = "R1")
        )[stra$gen, ]
    ),
    0
  )
    ...
    max(
      lme4::getME(model1, "Z")[rownames(stra), target_trms] -
        build_new_Z(
          model1,
          "gen",
          new_data = data.frame("loc" = "L2", "rep" = "R2")
        )[stra$gen, ]
    ),
    0
  )
    ...
    max(
      lme4::getME(model1, "Z")[rownames(stra), target_trms] -
        build_new_Z(
          model1,
          "gen",
          new_data = data.frame("loc" = "L3", "rep" = "R3")
        )[stra$gen, ]
    ),
    0
  )
  ...
  expect_equal(
    max(
      lme4::getME(model2, "Z")[rownames(stra), target_trms] -
        build_new_Z(
          model2,
          "gen",
          new_data = data.frame("loc" = "L1", "rep" = "R1")
...
# lettuce_lme4 <-  lme4::lmer(y ~  rep + (0 + loc | gen ) + (1| gen), data = lettuce_phenotypes)
