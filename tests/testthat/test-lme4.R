test_that("Reproduce lme4 Cullis", {
  ...
  reformulas::nobars_(formula(model_ran_lmer))
})
...
test_that("Reproduce lme4 Oakey", {
  ...
  X <- model_ran |>
    lme4::getME("X") |>
    as.matrix() # Design matrix fixed effects
  Z <- model_ran |>
    lme4::getME("Z") |>
    as.matrix() # Design matrix random effects
  ...
  C <- rbind(
    cbind(C11, C12),
    cbind(C21, C22)
  ) |> as.matrix() # Combine components into one matrix C
  ...
  H2Oakey.approx <- 1 - sum(diag(as.matrix(solve(G_g) %*% C22_g / n_g)), na.rm = TRUE)
