...
A case-specific wrapper for calculating broad / narrow sense
heritability.

- The lowercase prefix `h2_` refers to the wrapper or subfunctions e.g.
  [`h2_Oakey()`](https://anu-aagi.github.io/heritable/reference/H2_Oakey.md)
  for calculating narrow sense heritability
...
- source:
  ...
  searched in the global environment. Ignored for broad-sense and
  `lmerMod` methods
...
The following methods are currently implemented for narrow-sense
...
- `"Delta"`: \$\$H^2\_{\Delta ij} = 1 -
  \frac{PEV^{BLUP}\_{\overline\Delta ij}}{\operatorname{Var}(g_i -
  g_j)}\$\$

- `"Standard"`: \$\$H^2\_{Standard} = \frac{\operatorname{Var}(g_i -
  g_j)}{\operatorname{Var}(y_i.. - y_j..)}\$\$

- `"Reliability"`: \$\$\bar{r}^2 = \frac{1}{n_g}\sum\_{i=1}^{n_g}
  \left(1 - \frac{var(\hat{g}^{BLUP}\_i)}{var(g_i)}\right)\$\$

`"Reliability"` is not part of the default `method` set but can be
requested explicitly. Use
...
- `"Delta"`: \$\$H^2\_{\Delta ij} = 1 -
  \frac{PEV^{BLUP}\_{\overline\Delta ij}}{2\sigma^2_g}\$\$
...
For further details of a specific method - take a look at helpfile for
each subfunctions
