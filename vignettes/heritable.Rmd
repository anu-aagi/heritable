...
<img src="figs/unnamed-chunk-3-1.png" width="80%" />
</div>
...
- `h2` / `H2()` wrappers will return a named vector by `method`
- `h2_Oakey()` / `H2_Delta()` will return a numeric value
- `h2_Delta_by_genotype()` / `H2_Delta_by_genotype()` will return a named list according to the `target` variable
- `h2_Delta_pairwise()` / `H2_Delta_pairwise()` will return a symmetrical matrix for all pairwise combinations of `target`
